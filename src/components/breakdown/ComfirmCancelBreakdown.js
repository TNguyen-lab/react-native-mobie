...
export default function ComfirmCancelBreakdown({ open, onCancel, breakdown, onRefresh }) {
  const [reasonCancel, setReasonCancel] = useState("");
  const [loading, setLoading] = useState(false);

  const handleSubmit = async () => {
    if (!reasonCancel.trim()) {
      Alert.alert("Lỗi", "Vui lòng nhập lý do huỷ.");
      return;
    }
    try {
      setLoading(true);
      const res = await _unitOfWork.breakdown.comfirmCancelBreakdown({
        data: {
      ...
      >
        <View style={styles.container}>
          <View style={styles.header}>
            <Text style={styles.title}>Huỷ phiếu</Text>
          </View>

          <View style={styles.body}>
            <Text style={styles.label}>
              Lý do huỷ <Text style={styles.required}>*</Text>
            </Text>
            <TextInput
